...
|---|---|---|
| Cover / wordmark | `Inkmote.coverReveal(canvas, { wordmark, tagline? })` | — |
| Section / chapter title | `Inkmote.textReveal(canvas, h1El, { text, font, n, seed })` | `data-ink="title"` on h1 + `Inkmote.autoInit(document)` |
| Key number / stat | `Inkmote.statReveal(canvas, el, { value, n, seed })` | `data-ink="stat" data-value="95%"` |
...
— it wires `textReveal` automatically for `data-ink="title"` elements. Stat and image
slides need explicit JS calls.
...
Always call inside `document.fonts.ready.then(...)` — canvas text sampling requires
fonts to be loaded:
